...
my_addon = xbmcaddon.Addon('plugin.audio.deezerkodi')
...
logging.basicConfig(filename=logfile_path, format=log_format, datefmt='%Y-%m-%d %H:%M:%S', level=logging.INFO)
...
def show_playlist(playlist_data, reverse=True, album_infos={}):
    ...
    album_infos: dict of the form {'name': album_name, 'cover': album_cover}
    """
    items = []
    playlist = []
    ...
    # creating and/or clearing DB from previous playlist
...
def show_artist(id):
    """Show artist "content", albums, songs, top"""
    items = [
